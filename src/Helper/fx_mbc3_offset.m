function [MBC, matData, FAST_linData, VTK] = fx_mbc3_offset( FileNames, az_offset, ModeVizFileName ) 
...
if nb == 3
    ...
    for iaz = matData.NAzimStep:-1:1  
        ...
        cos_col_offset = cos(az(:) + az_offset);  % Jesse
        sin_col_offset = sin(az(:) + az_offset);  % Jesse

        tt  = [ones(3,1), cos_col, sin_col];        % Eq. 9, t_tilde
        tt_offset = [ones(3, 1), cos_col_offset, sin_col_offset];  % Jesse
    ...
    %     T1qc = eye(matData.NumHDInputs);            % inverse of T1q   
        ...
        for ii = 1:matData.n_RotTripletInputs
            T1c = blkdiag(T1c, tt_offset);  % Jesse
        end
